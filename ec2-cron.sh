...
# You need jq installed, aws cli and add proper policies to the EC2 role
...
asg=$(aws autoscaling describe-auto-scaling-instances --region $region --instance-ids $instanceID | grep AutoScalingGroupName|awk -F\" '{print $4}')

# Get all instances in the Auto Scaling Group with LifecycleState = InService
instances=$(aws autoscaling describe-auto-scaling-groups --region $region --auto-scaling-group-names $asg | jq '.AutoScalingGroups[0].Instances[] | select(.LifecycleState=="InService")' | jq -r .InstanceId)

# Iterate through all instances and check if we have a tag with name "cron" and its value is "on" or "off"
for instance in $instances
do
  cron=$(aws ec2 describe-tags --filters "Name=resource-id,Values=$instance" --region $region | jq '.Tags[] | select(.Key=="cron")' | jq -r .Value)
  if [ "$cron" == "on" ]; then
...
if [ $doWeHaveCronInstance -eq 0 ]; then
  ...
  aws ec2 create-tags --resources "$instanceID" --tags Key="cron",Value="on" --region $region
else
  # We only tag instance to off
  aws ec2 create-tags --resources "$instanceID" --tags Key="cron",Value="off" --region $region
fi
